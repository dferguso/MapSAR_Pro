...
def writeToOpPeriod(opPeriod, wEather):
    opInfo = os.path.join(wrkspc,"Operational_Period")
    fields = ['Weather', 'Wind']
    where = '"OpPeriod" = {0}'.format(str(opPeriod))
    
    with arcpy.da.UpdateCursor(opInfo,fields, where) as cursor:
        for row in cursor:
            row[0]=wEather['Forecast']
            row[1]=wEather['Wind']
...
if __name__ == '__main__':
    ...
    fc = os.path.join(wrkspc,"Planning_Points")
    ...
    with arcpy.da.SearchCursor(fc, fields, where, unProjCoordSys ) as cursor:
        for row in cursor:
            feat = row[0]
