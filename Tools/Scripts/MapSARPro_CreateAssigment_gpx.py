...
def joinCheck(FName, fc, df, TaskMap):       
    lyrs= df.listLayers(fc)
    for lyr in lyrs:
...
def ifExist(fClass, df,TaskMap, fc, symbologyLayer, SegArea_KM, SearchTime):
    if arcpy.Exists(fClass):
        ...
        ##check for joins
        where4 = joinCheck("Area_Name",fClass, df,TaskMap)
    
        rows4 = arcpy.da.SearchCursor(fClass, where4)
        for row4 in rows4:
            if fc != "none":
                ...
                if geometryType == 'Polygon':
                    ...
                    symbologyLayer = df.ListLayers(fClass)[0]
...
#    try:
    ...
    if fc == "none":
        ...
        try:
            reMoveLyr = df.ListLayers("FgTrzG")[0]
            df.RemoveLayer(reMoveLyr)
            del reMoveLyr
        except:
            pass
        ...
        mkLyr = df.Layer("FgTrzG")
        df.AddLayer(df,mkLyr,"TOP")
        updateLayer = df.ListLayers("FgTrzG")[0]
        df.UpdateLayer(updateLayer,symbologyLayer,True)
        selectLayer = df.ListLayers("FgTrzG")[0]
        ...
        try:
            ...
            mkPtLyr = df.Layer("FgTrzPt")
            df.AddLayer(df,mkPtLyr,"TOP")
            updateLayer002 = df.ListLayers("FgTrzPt")[0]
            ##Label Features
            ...
            ###########
            sLayer=df.ListLayers("Segment_Points")[0]
            df.UpdateLayer(updateLayer002,sLayer,True)
            del sLayer
        except:
            pass
        ##########################

        where4 = joinCheck("Area_Name",selectLayer, df,TaskMap)
        arcpy.SelectLayerByAttribute_management (selectLayer, "NEW_SELECTION", where4)
        
        #this is updating the map layout
        try:
            for llyr in df.ListLayers("*"):
                if str(llyr.name) == "MRGS_UTM_USNG":
                    mapLyr=df.ListLayers("MRGS_UTM_USNG")[0]
                elif str(llyr.name) == "MRGSZones_World":
                    mapLyr=df.ListLayers("MGRSZones_World")[0]
            arcpy.SelectLayerByLocation_management(mapLyr,"INTERSECT",selectLayer)
            rows7=arcpy.SearchCursor(mapLyr)
            row7 = rows7.next()
            if df.ListLayoutElements("TEXT_ELEMENT", "UTMZone"):
                UTMZn=df.ListLayoutElements("TEXT_ELEMENT", "UTMZone")[0]
                UTMZn.text = row7.getValue("GRID1MIL")
                UtmZone=UTMZn.text
                del UTMZn
            if df.ListLayoutElements("TEXT_ELEMENT", "USNGZone"):
                USNGZn=df.ListLayoutElements("TEXT_ELEMENT", "USNGZone")[0]
        ...
        try:
            ...
            if int(cIncident.getOutput(0)) > 0:
                mapLyr = df.ListLayers("Incident_Information")[0]
                if not MagDec:
                    arcpy.AddWarning("No Magnetic Declination provided in Incident Information")
                else:
                    if df.ListLayoutElements("TEXT_ELEMENT", "MagDecl"):
...
##          outFile = output + "/" + str(AssNum) + "_aerial.pdf"
        ...
        ## Add June 11, 2014
        try:
            reMoveLyr002 = df.ListLayers("FgTrzPt")[0]
            df.RemoveLayer(reMoveLyr002)
            del reMoveLyr002
...
def CreatingGPX(fc, df, TaskMap, AssNum, output, Assignment, Assign, OpPeriod):
    try:
        ...
        if fc == "none":
            ...
            rows6 = arcpy.da.SearchCursor(fc, where4, \
            ...
            filegpx = "{}/{}_GPX.gpx".format(output,str(AssNum))
            txt= open (filegpx, "w")
            txt.write('<?xml version="1.0" encoding="UTF-8"?>\n')
            txt.write('<gpx xmlns="http://www.topografix.com/GPX/1/1" version="1.1"'
                      ' creator="IGT4SAR" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation='
                      '"http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd '
                      'http://www.topografix.com/GPX/gpx_overlay/0/3 http://www.topografix.com/GPX/gpx_overlay/0/3/gpx_overlay.xsd '
                      'http://www.topografix.com/GPX/gpx_modified/0/1 http://www.topografix.com/GPX/gpx_modified/0/1/gpx_modified.xsd">\n')
            # Build variables for various statements in SARTopo Backup
            Status = 'PREPARED' #str(Assignment[AssNum][18])
            unrespPOD = ''
            OpPd =Assignment[AssNum][2]
            TaskInstruct='{}. Safety: {}'.format(str(Assignment[AssNum][3]), str(Assignment[AssNum][15]))
            ResourceType = ''
            preSearch = str(Assignment[AssNum][9])
            PrepBy = str(Assignment[AssNum][16])
            secFreq = ''
            Trans = ''
            respPOD = ''
            cluePOD = ''
            priFreq = str(OpPeriod[OpPd][2])
            timeAlloc = str(Assign[21])
            if not timeAlloc:
                timeAlloc=str(0.0)
            segName = str(Assignment[AssNum][10])
            ...
            if shpType == "Point":
                for row6 in rows6:
                    ...
                    #pnt = row6[0].getPart()
                    pnt = feat.getPart()
                    txt.write('<wpt lat="{}" lon="{}">\n'.format(str(pnt.Y),str(pnt.X)))
                    txt.write('<name>{}</name>\n'.format(str(AssNum)))
                    txt.write('<cmt>{}</cmt>\n'.format(segName))
                    txt.write('<desc><![CDATA[comments={}&url=%23FF0000]]></desc>\n'.format(TaskInstruct))
                    txt.write('<sym>WAYPOINT</sym>\n')
                    txt.write('<extensions>\n')
                    txt.write('<label xmlns="http://www.topografix.com/GPX/gpx_overlay/0/3">\n')
                    txt.write('<label_text>{}</label_text>\n'.format(str(AssNum)))
                    txt.write('</label>\n')
                    txt.write('</extensions>\n')
                    txt.write('</wpt>\n')
                    del feat
                ...
                for row6 in rows6:
                    txt.write('<name>{}</name>\n'.format(str(AssNum)))
                    txt.write('<number>{}</number>\n'.format(str(k)))
                    txt.write('<cmt>{}</cmt>\n'.format(segName))
                    textOut = ('status={}&operationalPeriodId={}&preparedBy={}&sartype=Assignment&previousEfforts={}&details={}&timeAllocated={}'
                               .format(Status,str(OpPd), PrepBy, PreSearch, TaskInstruct, timeAlloc))

#                    textOut = ('status={}&unresponsivePOD={}&operationalPeriodId={}&secondaryFrequency={}&transportation={}&preparedBy='
#                               '{}&resourceType={}&sartype=Assignment&id=1&previousEfforts={}&details={}&responsivePOD={}&cluePOD={}&primaryFrequency={}&timeAllocated={}'
#                               .format(Status, unrespPOD, str(OpPd), secFreq, Trans, PrepBy, ResourceType, PreSearch, TaskInstruct, respPOD, cluePOD, priFreq, timeAlloc))
                    txt.write('<desc><![CDATA[{}]]></desc>\n'.format(textOut))
                    txt.write('<extensions>\n')
                    txt.write('<label xmlns="http://www.topografix.com/GPX/gpx_overlay/0/3">\n')
                    txt.write('<label_text>{}</label_text>\n'.format(str(AssNum)))
                    txt.write('</label>\n')
                    txt.write('<gpxx:TrackExtension>')
                    txt.write('<gpxx:DisplayColor>Red</gpxx:DisplayColor>')
                    txt.write('</gpxx:TrackExtension>')
                    txt.write('</extensions>\n')
...
if __name__ == '__main__':
    ...
    if int(cAssign.getOutput(0)) > 0:
        ...
        rows3 = arcpy.da.SearchCursor(fc3)
        row3 = rows3.next()
...
###########################
    ...
    if int(cAssign.getOutput(0)) > 0:
        ...
        rows3 = arcpy.da.SearchCursor(fc8)
        row3 = rows3.next()
        k=0
        while row3:
...
##################################
    ...
    if int(cAssign.getOutput(0)) > 0:
        ...
        rows4 = arcpy.da.SearchCursor(fc9)
        row4 = rows4.next()
        k=0
        while row4:
...
##################################
    ...
    if int(cAssign.getOutput(0)) > 0:
        ...
        rows0 = arcpy.da.SearchCursor(fc2)
        row0 = rows0.next()
        while row0:
            ...
            Status=row0.getValue("Status")
            Status = 'Planned' if not Status else Status
            ...
            Assignment[AssignIdx]=[AssignNumber, PlanNumber, Period, TaskInstruct, Milage, Team,
                       ...
                       CreateGpx, CreateKml, Assign_Safety, PrepBy, TaskDate, Status]
...
################################################################################################
    ...
    for AssNum in AssignNum:
        ...
        SearchSpd = 0.0
        SearchTime = 0.0
        symbologyLayer = df.ListLayers("Search Boundary")[0]

##        try:
        if arcpy.Exists(fc4):
            ##check for joins
            where4 = joinCheck("Area_Name",fc4, df,TaskMap)
        ...
        fc, symbologyLayer, SegArea_KM, SearchTime = ifExist(fc5, df,TaskMap, fc, symbologyLayer, SegArea_KM, SearchTime)
        fc, symbologyLayer, SegArea_KM, SearchTime = ifExist(fc6, df,TaskMap, fc, symbologyLayer, SegArea_KM, SearchTime)
        fc, symbologyLayer, SegArea_KM, SearchTime = ifExist(fc7, df,TaskMap, fc, symbologyLayer, SegArea_KM, SearchTime)
        fc, symbologyLayer, SegArea_KM, SearchTime = ifExist(fc10, df,TaskMap, fc, symbologyLayer, SegArea_KM, SearchTime)
...
##########################################
    ...
    ##
        if Assign[12] == 'Yes':
            CreatingMap(fc, symbologyLayer, Assign, AssNum, df, MagDec, output)
        else:
            arcpy.AddMessage('No map created')

    ## Create gpx file for task
        if Assign[13] == 'Yes':
            CreatingGPX(fc, df, TaskMap, AssNum, output, Assignment, Assign, OpPeriod)
        else:
    ...
    ##Automate map production - July 27, 2012
        if Assign[14] == 'Yes':
            CreatingKML(fc, df, TaskMap, AssNum, output)
        else:
