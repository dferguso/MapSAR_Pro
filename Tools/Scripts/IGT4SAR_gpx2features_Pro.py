...
def gpxToPoints(df, gpxfile, outFC, inCoord, outCoord, refGroupLayer, datumTrans):
    ...
    #
    arcpy.AddMessage("filepath: {}".format(os.path.dirname(outFC)))
    arcpy.CreateFeatureclass_management(os.path.dirname(outFC), 'GpxPts_Temp', 'POINT', '', 'DISABLED', 'ENABLED', inCoord)
    inFC = os.path.join(os.path.dirname(outFC),'GpxPts_Temp')
    ...
    for index, trkPoint in enumerate(GeneratePointFromXML(tree)):
      ...
      if trkPoint.asPoint() is not None:
        ...
        cursor.insertRow([NAME, Descript, iTYPE, Date_Time, Elevation, ptGeo])
...
def Point2Line(df, outFC, outCoord, refGroupLayer, GPS_Pt_Lyr):
    ...
    featureName = os.path.basename(outFC).partition('.shp')[0]
    featureLineName = featureName + '_line'
    outputLocationLine = os.path.dirname(outFC) + '\\' + featureLineName
...
if __name__ == "__main__":
    ...
    wrkFldr= os.path.dirname(wrkspc)
    ...
    outFC=os.path.join(wrkspc, outFC)
