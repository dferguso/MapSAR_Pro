...
if __name__ == '__main__':
    ...
    OpTime = now.strftime("%m/%d/%y %I:%M %p")
...
#        edit.stopEditing(True)
    ...
    if LeadAgency: 
        ...
        with arcpy.da.InsertCursor(domTable,[codeField]) as cursor:
            cursor.insertRow([keyName])
        del cursor
               
        xChk = checkDomain(wrkspc, domTable , keyName)
        if not xChk:
            msg="Update {} domain".format(TbleText)
            arcpy.AddMessage(msg)
            try:
...
# #    arcpy.AddMessage("\n")
    ...
    if IncidName:
        df.name = IncidName
        IncidChk = True
        # Set the name of the main dataframe to the Incident Name
    elif IncidNum:
        df.name = IncidNum
        IncidChk = True
    ...
    if IncidChk == True:
        ...
        updateOp = "REPLACE" 
        keyName = keyName
        flds=['IncidName','IncidNum','MapDatum','ICS204','IncidID']          
        if arcpy.Exists(IncidInfo):
            IncidID=subGUID='{' + str(uuid.uuid4()).upper() + '}'
            cursor = arcpy.da.InsertCursor(domTable,flds)
            if not IncidName:
                IncidName=''
            if not IncidNum:
                IncidNum=''
...
###############################################
    ...
    TbleText = "Operation Period Information"
    domTable = OpInfo
    codeField = "OpPeriod"
    descField = "OpPeriod"
    domName = "Period"
    ...
    updateOp = "REPLACE" 
    keyName = keyName
    flds=['OpPeriod', 'Start_Date']
        
    OpPeriod = 1
    with arcpy.da.InsertCursor(domTable,flds) as cursor:
