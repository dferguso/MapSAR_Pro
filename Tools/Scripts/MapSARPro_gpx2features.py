...
def gpxToPoints(df, gpxfile, outFC, inCoord, outCoord, refGroupLayer, datumTrans):
    ...
    #
    arcpy.AddMessage("filepath: {}".format(path.dirname(outFC)))
    arcpy.CreateFeatureclass_management(path.dirname(outFC), 'GpxPts_Temp', 'POINT', '', 'DISABLED', 'ENABLED', inCoord)
    inFC = path.join(path.dirname(outFC),'GpxPts_Temp')
    ...
    for props in fieldList:
      ...
      fldList.append(props[0])

    fldList.append("SHAPE@")
    pt=arcpy.Point()
    cursor = arcpy.da.InsertCursor(inFC,fldList)
    ...
    for index, trkPoint in enumerate(GeneratePointFromXML(tree)):
      ...
      if trkPoint.asPoint() is not None:
        NAME = trkPoint.name
        Descript = trkPoint.desc
        iTYPE = trkPoint.gpxtype
        pt.X=float(trkPoint.x)
        pt.Y=float(trkPoint.y)
        pt.Z=float(trkPoint.z)
        ptGeo=arcpy.PointGeometry(pt, inCoord)        
        Elevation = trkPoint.z
        Date_Time = trkPoint.t

        cursor.da.insertRow([NAME, Descript, iTYPE, Date_Time, Elevation, ptGeo])
    ...
    GPS_Pt_Lyr = arcpy.MakeFeatureLayer_management(outFC, "GPX_pts").getOutput(0)
    df.addLayerToGroup(refGroupLayer,GPS_Pt_Lyr,'TOP')

    arcpy.Delete_management(inFC)
    
    Point2Line(df, outFC, outCoord,refGroupLayer, GPS_Pt_Lyr)

    return()


def Point2Line(df, outFC, outCoord, refGroupLayer, GPS_Pt_Lyr):
    # Convert the point layer to lines
    featureName = path.basename(outFC).partition('.shp')[0]
    featureLineName = featureName + '_line'
    outputLocationLine = path.dirname(outFC) + '\\' + featureLineName

    arcpy.AddMessage('\n')
    whereClause = "Type = 'TRKPT'"
    arcpy.SelectLayerByAttribute_management(GPS_Pt_Lyr, "NEW_SELECTION", whereClause)
    arcpy.PointsToLine_management(Input_Features=outFC, Output_Feature_Class=outputLocationLine,Line_Field="Name",Sort_Field="#",Close_Line="NO_CLOSE")

    GPS_Line_Lyr = arcpy.MakeFeatureLayer_management(outputLocationLine, "GPX_lines").getOutput(0)   
    df.addLayerToGroup(refGroupLayer,GPS_Line_Lyr,'BOTTOM')
...
if __name__ == "__main__":
    ...
    try:
        refGroupLayer=df.listLayers('*GPS_Tracks_And_Routes*')[0]
    except:
        refGroupLayer=""

    outFC=path.join(wrkspc, outFC)
    gpxToPoints(df, gpxfile, outFC, inCoord, outCoord, refGroupLayer,datumTrans)
